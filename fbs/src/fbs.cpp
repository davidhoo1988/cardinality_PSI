 ...
 */
    ...
    // this rotation polynomial is set up for half-domain bootstrap only
    lbcrypto::NativePoly constructRotationPolynomial2(const std::shared_ptr<RingGSWCryptoParams>& params, const BootstrapFunction& F) {

        auto rotP = lbcrypto::NativePoly(params->GetPolyParams(), COEFFICIENT, true);
        ...
        int p = 2 * int(NF / T); // p = 2N/t
        int bound = NF / T;

        for(uint32_t i = 0; i < Td2; i++) {
            uint32_t center = i * p;

            for(int e = -bound; e < bound; e++) {
                int pos = (int(center) + e) % NF;
                if (center == 0) { // special point, used for f(0)
                    // ORIG rotP1[pos] = Q - F(TD2)
                    if (e == 0)
                        // rotP0[pos] = (Q-F(Td2)) % Q;
                        rotP[pos] = F(0);
    ...
    std::shared_ptr<LWECiphertextImpl> RingGSWAccumulatorScheme::HalfDomainBootstrap(
        ...
        auto rotP = constructRotationPolynomial2(params, bootsMAP);
        // std::cout << rotP << std::endl;
        (*accu)[0][0] = std::move(zero);
        (*accu)[0][1] = std::move(rotP);

...
public:
   ...
   inline void insert(const unsigned char* key_begin, const std::size_t& length)
   {
      ...
      for (std::size_t i = 0; i < salt_.size(); ++i)
      {
         ...
         // std::string str = "(" + std::to_string(bit_index / bits_per_char) + ", " + std::to_string(bit) + ")  ";
         // std::cout << str; 
         // std::cout << bit_index << "\t";
      }
      // std::cout << std::endl; 
   ...
   inline void insert(const InputIterator begin, const InputIterator end)
   {
    ...
    // user defined function, generate bit table address for the given item
   inline void gen_indices(const unsigned char* key_begin, const std::size_t& length, std::vector<std::size_t> &indices_list)
   {
      std::size_t bit_index = 0;
      std::size_t bit       = 0;
      ...
      for (std::size_t i = 0; i < salt_.size(); ++i)
      {
         compute_indices(hash_ap(key_begin, length, salt_[i]), bit_index, bit);   
         // bit_table_[bit_index / bits_per_char] |= bit_mask[bit];
         indices_list[i] = bit_index;
         // std::cout << "i: " << i << ", indices_list[i] " << indices_list[i] << std::endl;
      }

   }

   template <typename T>
   inline void gen_indices(const T& t, std::vector<std::size_t> &indices_list)
   {
      // Note: T must be a C++ POD type.
      gen_indices(reinterpret_cast<const unsigned char*>(&t), sizeof(T), indices_list);
   }

   inline void gen_indices(const std::string& key, std::vector<std::size_t> &indices_list)
   {
      gen_indices(reinterpret_cast<const unsigned char*>(key.data()),key.size(), indices_list);
   }

   inline void gen_indices(const char* data, const std::size_t& length, std::vector<std::size_t> &indices_list)
   {
      gen_indices(reinterpret_cast<const unsigned char*>(data), length, indices_list);
   }

   template <typename InputIterator>
   inline void gen_indices(const InputIterator begin, const InputIterator end)
   {
      InputIterator itr = begin;
